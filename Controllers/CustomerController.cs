...
namespace ShopAI.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        [HttpGet("Insights/{sellerId}")]
        public async Task<ActionResult<CustomerInsightDTO>> GetCustomerInsights(int sellerId)
        {
            var result = await _customerService.GetCustomerInsightsAsync(sellerId);
            return Ok(result);
        }

        [HttpGet("WithOrders/{sellerId}")]
        public async Task<ActionResult<List<CustomerWithOrdersDTO>>> GetCustomerWithOrders(int sellerId)
        {
            var result = await _customerService.GetCustomerOrderSummaryAsync(sellerId);
            return Ok(result);
        }
    }
}
